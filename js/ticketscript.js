...
const validateForm = () => {
    ...
    if(cantidadTickets.value === "" || isNaN(cantidadTickets.value) || cantidadTickets.value <= 0){
        cantidadTickets.classList.add("is-invalid");
        divErrorCantidad.classList.add("mostrarError");
        cantidadTickets.focus();
        return;
    }
    let precioTicket = valorTicket;
    switch(categoria.value){
        case "estudiante":
            precioTicket -= dtoEstudiante;
            break;
        case "trainee":
            precioTicket -= dtoTrainee;
            break;
        case "junior":
            precioTicket -= dtoJunior;
            break;
    }
    let total = precioTicket * cantidadTickets.value;
